 ...
 */
package com.e.bloctap2pay.nfc.parser;

import android.util.Log;

import com.e.bloctap2pay.nfc.enums.CommandEnum;
...
import com.e.bloctap2pay.nfc.iso7816emv.EmvTerminal;
import com.e.bloctap2pay.nfc.iso7816emv.TLV;
import com.e.bloctap2pay.nfc.iso7816emv.TagAndLength;
import com.e.bloctap2pay.nfc.model.Afl;
import com.e.bloctap2pay.nfc.model.EmvCard;
import com.e.bloctap2pay.nfc.model.EmvTransactionRecord;
import com.e.bloctap2pay.nfc.model.enums.CurrencyEnum;
import com.e.bloctap2pay.nfc.utils.CommandApdu;
import com.e.bloctap2pay.nfc.utils.ResponseUtils;
import com.e.bloctap2pay.nfc.utils.TlvUtil;
import com.e.bloctap2pay.nfc.utils.TrackUtils;
...
public class EmvParser {
	...
	//	byte[] data2 = provider.transceive("80 A8 00 00 0A 83 08 00 00 00 00 00 01 00 00 00".getBytes(StandardCharsets.UTF_8));
//		byte[] data2 =provider.transceive(new CommandApdu(CommandEnum.GPO, out.toByteArray(), 0).toBytes());
		if (ResponseUtils.isSucceed(data)) {
			// Extract PIN try counter

			byte[] val = TlvUtil.getValue(data, EmvTags.PIN_TRY_COUNTER);
//			byte[] Unpredictable  = TlvUtil.getValue(data, EmvTags.C);
			if (val != null) {
//				Log.d("EmvParser","Updic=>>>>>>>>"+  BytesUtils.bytesToString(Unpredictable)  );
	...
	protected boolean parse(final byte[] pSelectResponse, final IProvider pProvider) throws CommunicationException {
		...
		byte[] upd = TlvUtil.getValue(gpo, EmvTags.UNPREDICTABLE_NUMBER);
		Log.d("EmvParser", "Upp++>>"+ BytesUtils.bytesToString(gpo));
	...
	protected void extractCardHolderName(final byte[] pData) {
		...
		String data = BytesUtils.bytesToStringNoSpace(cardHolderByte);

		if (data==null || data.isEmpty()){
			byte[] track1 = TlvUtil.getValue(pData,EmvTags.TRACK1_DATA);
			 cardHolderName = getCardHolderFromTrack1(BytesUtils.bytesToStringNoSpace(track1));
			Log.d("EmvParser", "CardHolder   =>>"+ cardHolderName);
		}
		...
		try {
			...
			if (list != null) {
				for (TagAndLength tl : list) {
					byte[] response = EmvTerminal.constructValue(tl);
					out.write(response);
					TlvUtil.setTagsRequest(BytesUtils.bytesToString(tl.getTag().getTagBytes()).replace(" ", ""), BytesUtils.bytesToString(response).replace(" ", ""));

				}
			}
			/*if (list != null) {
				for (TagAndLength tl : list) {
//					byte ret[] = new byte[tl.getLength()];
//				  	random.nextBytes(ret)

					out.write(EmvTerminal.constructValue(tl, card));
